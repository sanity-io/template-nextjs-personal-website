...
- Running the app: `npm run dev` (site at http://localhost:3000, Studio at http://localhost:3000/studio). `predev` runs `sanity schema extract --force` (regenerates the committed `schema.json`/`sanity.types.ts` byte-identically) followed by `sanity typegen generate`. The `--force` flag is required because `schema.json` is committed and `extract` otherwise refuses to overwrite it. To skip typegen entirely, run `npx next dev --turbopack` directly.
