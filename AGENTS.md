...
- Running the app: `npm run dev` (site at http://localhost:3000, Studio at http://localhost:3000/studio). It runs Sanity typegen at startup so `sanityFetch` query result typings are current. To skip typegen entirely, run `npx next dev --turbopack` directly.
