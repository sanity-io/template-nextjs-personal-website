...
- RUN: `npx playwright test` (script: `npm run test:e2e`) against `BASE_URL` (default `http://localhost:3000`). Playwright does not start the server; the LOOP does.
...
- WALLS:
  ...
  - GitHub CI (`.github/workflows/ci.yml`) runs `type-check` and `lint` only; it does not have Sanity secrets or Playwright browsers, so this loop is local.
